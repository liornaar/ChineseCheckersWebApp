...
import java.util.Random; //on comment for now
...
public class GameEngine {
    ...
    private Point currentPickedMarblePoint = new Point();
    
    public Point getCurrentPickedMarble(){
        return currentPickedMarblePoint;
    }
    
    public void setCurrentPickedMarble(Point pointToSet){
        currentPickedMarblePoint.setLocation(pointToSet);
    }
...
//        return players.get(randomNum);
    ...
    public void calculatePossibleMovesForPlayer(Point pickedMarble) {
        ...
        currentPickedMarblePoint.setLocation(pickedMarble);
        addOrRemovePossibleMovesInBoard(false);
        ...
        addOrRemovePossibleMovesInBoard(true);
    }
    ...
    public void updateBoardWithNewPoint(Point newMarble) {

        Color color = board.getColorByCord(currentPickedMarblePoint.x, currentPickedMarblePoint.y);
        char c = board.getCharByCord(currentPickedMarblePoint.x, currentPickedMarblePoint.y);
        
        board.setCharByCord(CheckersBoard.EMPTY, Color.EMPTY, currentPickedMarblePoint.x, currentPickedMarblePoint.y);
    ...
    public void switchTurns() {
        currentPlayersTurn.incNumberOfMoves();
        addOrRemovePossibleMovesInBoard(false);
...
//        updatePlayerMarblesAfterMove(bestSourceMarble, bestDestinationMarble);
    ...
    public void updatePlayerMarblesAfterMove(Point userChoice) {
        currentPlayersTurn.getMarbles().remove(currentPickedMarblePoint);
    ...
    public void calculatePossibleSkipPointsOnly(Point pickedUpMarble) {
        currentPickedMarblePoint.setLocation(pickedUpMarble);
        addOrRemovePossibleMovesInBoard(false);
        possibleMovesForCurrentPickedMarble.clear();
    ...
    public void removeCurrentPlayerFromGame(String playerName) {
        for (Player player : players) {
    ...
    public void addOrRemovePossibleMovesInBoard(boolean toAdd) {
        if (toAdd) {//paint all moves
            for (Point possibleMove : possibleMovesForCurrentPickedMarble) {
                board.setCharByCord(EMPTY, Color.POS_MOVE, possibleMove.x, possibleMove.y);
            }
        } else {//clear all painted moves
            for (Point possibleMove : possibleMovesForCurrentPickedMarble) {
                board.setCharByCord(EMPTY, Color.EMPTY, possibleMove.x, possibleMove.y);
            }
        }
    }
}
